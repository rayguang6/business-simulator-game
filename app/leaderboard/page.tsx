  ...
  const renderLeaderboardTable = (
    ...
    return (
      <div className="w-full max-w-4xl bg-slate-800 shadow-2xl rounded-xl overflow-hidden mb-12 border border-slate-700">
        <h2 className="text-2xl sm:text-2xl md:text-3xl font-semibold text-center text-indigo-300 py-4 bg-slate-700/50 whitespace-nowrap overflow-hidden text-ellipsis">{title}</h2>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[480px] text-left text-xs sm:text-sm md:text-base">
            <thead className="bg-slate-700/50 border-t border-b border-slate-600">
              <tr>
                <th className="p-1 sm:p-2 md:p-3 font-semibold text-indigo-300">Rank</th>
                <th className="p-1 sm:p-2 md:p-3 font-semibold text-indigo-300">Player</th>
                <th className="p-1 sm:p-2 md:p-3 font-semibold text-indigo-300 text-center">Months{type === 'mostMonths' ? ' Survived' : ''}</th>
                {type === 'mostMonths' && (
                  <th className="p-1 sm:p-2 md:p-3 font-semibold text-indigo-300 text-right">Highest Cash Reached</th>
                )}
                <th className="p-1 sm:p-2 md:p-3 font-semibold text-indigo-300">Industry</th>
              </tr>
            </thead>
              ...
              {data.length > 0 ? (
                data.map((entry) => (
                  <tr key={entry.rank} className="hover:bg-slate-700/30 transition-colors duration-150">
                    <td className="p-1 sm:p-2 md:p-3 font-medium text-base sm:text-lg whitespace-nowrap">
                      {entry.rank === 1 ? '🥇' : entry.rank === 2 ? '🥈' : entry.rank === 3 ? '🥉' : entry.rank}
                    </td>
                    <td className="p-1 sm:p-2 md:p-3 text-slate-200 font-semibold whitespace-nowrap">{entry.display_name || '-'}</td>
                    <td className="p-1 sm:p-2 md:p-3 text-sky-400 font-bold text-center whitespace-nowrap">{entry.months_played ?? 'N/A'}</td>
                    {type === 'mostMonths' && (
                      <td className="p-1 sm:p-2 md:p-3 text-right font-mono text-amber-300">
                        ${(entry.highest_cash ?? entry.final_cash ?? 0).toLocaleString()}
                      </td>
                    )}
                    <td className="p-1 sm:p-2 md:p-3 text-slate-400 whitespace-nowrap">
                ...
                <tr>
                  <td colSpan={colSpan} className="p-3 sm:p-4 text-center text-slate-400 text-xs sm:text-sm md:text-lg">
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-4 sm:p-8 flex flex-col items-center">
      ...
      <header className="w-full max-w-4xl mb-8 text-center">
        <h1 className="text-3xl sm:text-4xl font-bold text-indigo-400 mb-2 whitespace-nowrap overflow-hidden text-ellipsis">🏆 Leaderboards 🏆</h1>
        <p className="text-slate-400 text-xs sm:text-sm">See who's at the top of the game!</p>
      </header>
